...
- `host_restic` models backup policy as targets, logical jobs, and feature contributions
- `host_restic` supports SSH-key-backed SFTP and target-specific environment variables
- `host_restic` supports job-scoped pre/post backup command hooks for service quiesce
- `host_restic` defaults `restic_apt_cache_valid_time` to `86400` and expects backup playbooks to skip unused fact gathering for faster repeat converges
